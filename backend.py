...
def AskFriendship(user_id ,friend_id): #use this for new friends request 
    r.sadd('user:'+str(friend_id)+".friendsRequestsWaiting",user_id)
    r.sadd('user:'+str(user_id)+".friendsRequestsSent",friend_id)

def AcceptRequest(user_id, friends_id):    
     r.sadd('user:'+str(user_id)+".friends", friends_id)
     r.sadd('user:'+str(friends_id)+".friends", user_id)
     r.srem('user:'+str(friends_id)+".friendsRequestsWaiting",friends_id)
...
def UpdateStatus(user_id, newStatut):
     ...
     r.sadd('user:'+str(user_id)+".updates", str(newStatut))  #check this list for the latest status


def NewPost(user_id, newPost): 
     r.sadd('user:'+str(user_id)+".posts",str(newPost))
     r.sadd('user:'+str(user_id)+".updates", str(newPost))

def get_all_posts(): 
    user_id = 1
    #user = "user:" + str(user_id)
    while r.hget("user:" + str(user_id),"username") != None:
        current_user =(str(r.hget("user:" + str(user_id),"username")))
        print(r.smembers("user:"+str(user_id)+".updates"))
        user_id+=1
...
print("Ask friend 2")
AskFriendship(1,2)
print(r.smembers("user:1.friendsRequestsSent"))
print("Accept Imad")
print(r.smembers("user:2.friendsRequestsWaiting"))
AcceptRequest(2,1)
print(r.smembers("user:1.friends"))

UpdateStatus(1,'hello my status is that')
NewPost(1,'this is a new post')
NewPost(1,'this is a new post 2')
print("fil d'actualité")
get_all_posts()
